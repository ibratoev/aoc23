...
fn part2() {
    let mut sum = 0;
    for line in INPUT_PART_1.lines() {
        let v = line.split(':').collect::<Vec<&str>>();
        let [_game, sets] = <[&str; 2]>::try_from(v).ok().unwrap();

        let sets = sets.split(';').collect::<Vec<&str>>();
        let mut max_green = 0;
        let mut max_red = 0;
        let mut max_blue = 0;
        for set in sets {
            ...
            for cube in cubes {
                if cube.contains("green") {
                    let num = cube.replace("green", "").trim().parse::<i32>().unwrap();
                    max_green = if num > max_green { num } else { max_green };
                } else if cube.contains("red") {
                    let num = cube.replace("red", "").trim().parse::<i32>().unwrap();
                    max_red = if num > max_red { num } else { max_red };
                } else if cube.contains("blue") {
                    let num = cube.replace("blue", "").trim().parse::<i32>().unwrap();
                    max_blue = if num > max_blue { num } else { max_blue };
                }
            }
        }
...
fn part1() {
    let mut sum = 0;
    'game_loop: for line in INPUT_PART_1.lines() {
        let v = line.split(':').collect::<Vec<&str>>();
        let [game, sets] = <[&str; 2]>::try_from(v).ok().unwrap();
        let game = game.replace("Game ", "").parse::<i32>().unwrap();
