...
fn part1() {
    ...
    for line in INPUT_PART_1.lines() {
        ...
        sum += char::to_digit(*chars.first().unwrap(), 10).unwrap() * 10;
        sum += char::to_digit(*chars.last().unwrap(), 10).unwrap()
    }
...
fn part2() {
    let mut sum = 0;
    for line in INPUT_PART_2.lines() {
        let l = line
        ...
        let chars: Vec<char> = l.chars().filter(|&c| c.is_digit(10)).collect();
        sum += char::to_digit(*chars.first().unwrap(), 10).unwrap() * 10;
        sum += char::to_digit(*chars.last().unwrap(), 10).unwrap()
    }
...
const INPUT_PART_1: &str = "29lzrxseven
9nnqljsixkzphvtmtr
...
const INPUT_PART_2: &str = "29lzrxseven
9nnqljsixkzphvtmtr
five73kskpfgbkcltwoccvf
fiveq7
npfdbnjlthree6cmdbsrsqc9five
